...
def user_ask_stuff():
  """
  Asks the user for the number of tries, starting number, and stopping number. Then asks the user whether to run different guessing options. Then runs the given option.
  """
  while True:
    try:
  ...
  while True:
    print(f"""
    Your options were tries: {tries}, start: {start}, stop: {stop}
    """)
    try:
      user_choice = int(input("What option would you like to try? "))
      if user_choice == 1:
        ...
        print("You decided to have the computer guess using a binary search. Good choice. I'll take over the world tomorrow.")
        guess_random_num_binary(tries, start, stop)
        break
      else:
        print("Why didn't you pick one of the right options??")
    except ValueError:
      print("Why would you put anything other than a whole number. You are such a human.")
...
def gambling_game():
  ...
  while True:
    ...
    win = guess_random_num_linear(5, 1, 10)
    if win == True:
      print("Congrats! You've won the bet.💵 \n")
      bet = 2 * bet
      player_money += bet
    if player_money <= 0:
      print("You've run out of money. Get a job and get some more to play again. Learn to code in Python.")
      break
    if player_money > 50:
      print(
    ...
    try:
      while True:  # <= Bonus Task 1
        ...
        if guess > stop or guess < start:
          ...
          print("Why would you guess a number you've already guessed? We told you that was wrong! Try again...")
        else:
